<?php
    ...
    if(strlen($_POST["kode"])==0) {
        ...
        $data = $db->getFieldValue("produk", array("COUNT(*) AS TOTAL"), array("kode" => $_POST["kode"]));
        if($data[0]["TOTAL"]>0) {
            $db->mysql_close();
    ...
    foreach ($_FILES as $key => $file) {
        if (strlen($file["name"])>0) {
            move_uploaded_file($file['tmp_name'], realpath($root) . "/uploads/produk/".$file['name']);

            $count_file = count($filename);
            $filename[$count_file] = $file['name'];
