<?php
    ...
    //INSERT CATEGORY
    $db->mysql_delete("konten_to_category", array("id_konten" => $_POST["id"]));
    ...
    //INSERT BANNER
    $db->mysql_delete("konten_to_banner", array("id_konten" => $_POST["id"]));
    ...
    $data = $db->getFieldValue("konten", array("banner", "thumbnail"), array("id" => $_POST["id"]));
    ...
    if(file_exists(realpath($root)."/uploads/konten/banner/".$data[0]["banner"])) {
        unlink(realpath($root)."/uploads/konten/banner/".$data[0]["banner"]);
    }
    ...
    if(file_exists(realpath($root)."/uploads/konten/thumbnail/".$data[0]["thumbnail"])) {
        unlink(realpath($root)."/uploads/konten/thumbnail/".$data[0]["thumbnail"]);
    }

    //$_POST["tanggal_update"] = date("Y-m-d H:i:s");
    
    //UPDATE DATA KONTEN
    $db->mysql_update("konten", $_POST, array("id" => $_POST["id"]));   
    $_SESSION["alert_success"] = "Edit data konten berhasil.";
    $db->mysql_close();
