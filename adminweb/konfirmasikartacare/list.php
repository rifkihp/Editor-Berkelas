<?php
    ...
    $totalrecord = $db->getFieldValue("kartacare_konfirmasi AS A LEFT JOIN member AS B ON A.id_member=B.id LEFT JOIN kartacare_donasi AS D ON A.id_donasi=D.id LEFT JOIN kartacare AS E ON D.id_kartacare=E.id", array("COUNT(*) AS TOTAL"), array(),  array(), 0, 0, "", " 1 " . 
        ...
        (strlen($query)>0?" AND (B.first_name LIKE '%$query%' OR  B.last_name LIKE '%$query%' OR B.email LIKE '%$query%' OR A.nama_bank_pengirim LIKE '%$query%' OR A.nama_pemilik_rekening LIKE '%$query%' OR E.judul LIKE '%$query%')":""));
    ...
    $listdetail = $db->getFieldValue("kartacare_konfirmasi AS A LEFT JOIN member AS B ON A.id_member=B.id LEFT JOIN bank_transfer AS C ON A.bank_tujuan=C.id LEFT JOIN kartacare_donasi AS D ON A.id_donasi=D.id LEFT JOIN kartacare AS E ON D.id_kartacare=E.id", array("A.id", "E.judul", "DATE_FORMAT(A.tanggal_jam, '%d-%m-%Y %H:%i:%s') AS tgl_jam", "A.nama_bank_pengirim AS bank_pengirim", "A.nama_pemilik_rekening AS nama_pengirim", "A.jumlah_transfer", "B.last_name", "B.first_name", "IF(B.email IS NULL, 'Guest', B.email) AS email", "A.status", "C.nama_pemilik_rekening AS nama_penerima", "C.nama_bank AS bank_tujuan", "A.photo"), array(), array(), $limit, $limit*($currentpage-1), " A.tanggal_jam DESC ",  " 1 " .
        (strlen($tanggal_dari)>0?" AND DATE_FORMAT(A.tanggal_jam, '%Y-%m-%d')>='$tanggal_dari' ":"") .
        (strlen($tanggal_sampai)>0?" AND DATE_FORMAT(A.tanggal_jam, '%Y-%m-%d')<='$tanggal_sampai' ":"") .
        ($filter_status<3?" AND A.status='$filter_status' ":""). 
        (strlen($query)>0?" AND (B.first_name LIKE '%$query%' OR  B.last_name LIKE '%$query%' OR B.email LIKE '%$query%' OR A.nama_bank_pengirim LIKE '%$query%' OR A.nama_pemilik_rekening LIKE '%$query%' OR E.judul LIKE '%$query%')":"")); 
    ...
    $db->mysql_close();
    $smarty->display(BACK_END_TEMPLATE_PATH."/konfirmasikartacare/list.html");    