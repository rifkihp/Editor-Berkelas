<?php
    ...
    $listdetail = $db->getFieldValue("category AS A LEFT JOIN category AS B ON A.id_parent=B.id", array("A.id", "A.nama", "A.penjelasan", "B.nama AS nama_parent", "A.header"), array(), array(), $limit, $limit*($currentpage-1), " id DESC ",  " 1 " . (strlen($query)>0?" AND (A.nama LIKE '%$query%' OR  A.penjelasan LIKE '%$query%')":"")); 
    ...
    $smarty->assign("title", "Kategori");
    $smarty->assign("sub_title", "Daftar Kategori");
