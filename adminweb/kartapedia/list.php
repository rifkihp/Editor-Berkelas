<?php
    ...
    $totalrecord = $db->getFieldValue("kartapedia AS A", array("COUNT(*) AS TOTAL"), array(),  array(), 0, 0, "", " 1 " . (strlen($query)>0?" AND (A.nama LIKE '%$query%' OR A.penjelasan LIKE '%$query%')":""));
    ...
    $listdetail = $db->getFieldValue("kartapedia AS A LEFT JOIN kartapedia AS B ON A.id_parent=B.id", array("A.id", "A.nama", "A.penjelasan", "B.nama AS nama_parent", "A.header"), array(), array(), $limit, $limit*($currentpage-1), " id DESC ",  " 1 " . (strlen($query)>0?" AND (A.nama LIKE '%$query%' OR  A.penjelasan LIKE '%$query%')":"")); 
    ...
    $db->mysql_close();
    $smarty->display(BACK_END_TEMPLATE_PATH."/kartapedia/list.html");    